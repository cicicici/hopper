...
def packet_to_string(packet):
    return packet.str_data()

def string_to_packet(str_data):
    return DataPacket(str_data.encode('utf-8'))
...
class DataLink(object):
    ...
    def is_server(self):
        return self._host is None or self._host == ''
    ...
    def handle_client_connection(self, client_id, client_socket):
        ...
        while self._running:
            try:
                ...
                if len(recv_data) == 0:
                    log.info(log.DC.NET, '[Server] Client disconnected, {}'.format(client_id))
                    client_socket.close()
    ...
    def server_listen(self):
        while self._running:
            try:
                ...
                log.info(log.DC.NET, '[Server] Accepted connection from {}'.format(client_id))
                ...
                log.info(log.DC.NET, '[Server] Cannot accept connection. Shutting down.')
                self._running = False
                pass
    ...
    def client_listen(self):
        ...
        while self._running:
            try:
                ...
                if len(recv_data) == 0:
                    log.info(log.DC.NET, '[Client] Server disconnected')
                    self._client.close()
                    break
                else:
                    recv_bytes += recv_data
        ...
        try:
            if self.is_server():
                for k, c in self._clients.items():
                    c.send(string_to_packet(str_data).pack())
            else:
                self._client.send(string_to_packet(str_data).pack())
        except:
            return False
    ...
    def send_sock(self, socket, str_data):
        ...
        try:
            socket.send(string_to_packet(str_data).pack())
        except:
            return False
    ...
    def close(self):
        ...
        try:
            if self.is_server():
                # Create a dummy client to localhost to terminate accept() thread
                dummy_link = DataLink(name='dummy', host='localhost', port=self._port)
                dummy_link.close()
                # Close all clients
                for k, c in self._clients.items():
                    c.close()
